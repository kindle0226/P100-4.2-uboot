...
int i2c_read(uchar chip, uint addr, int alen, uchar * buffer, int len)
{
 ...
 int i2c_xfer (uchar chip, u8 * data, int data_size, enum i2c_xfer_flag xfer_mode)
 {
	u16 stat;
	u16 w;
	u8 data_x;
	...
	outw(w, I2C_BUF);
	/* stop bit needed here */
	w = I2C_CON_EN | I2C_CON_MST | I2C_CON_STT  | I2C_CON_STP;
	if (xfer_mode == I2C_XFER_M_WR)
		w |= I2C_CON_TRX;
	outw(w, I2C_CON);
	
	while ((stat = wait_for_pin() )){
		if (count++ == 100) {
			printf("Too much work in one writing task\n");
			break;
		}
		 ...
		 */
		outw(stat &
				~(I2C_STAT_RRDY | I2C_STAT_RDR |
				I2C_STAT_XRDY | I2C_STAT_XDR), I2C_STAT);
		...
		if (stat & (I2C_STAT_RRDY | I2C_STAT_RDR)) {
			...
			if (data_size) {
				*data ++ = data_x;
				...
				if (stat & I2C_STAT_RRDY)
					printf("RRDY IRQ while no data requested\n");
				if (stat & I2C_STAT_RDR)
					printf("RDR IRQ while no data requested\n");
				return 1;
			}

			outw(stat & (I2C_STAT_RRDY | I2C_STAT_RDR), I2C_STAT);
			continue;
		}
			...
			while (num_bytes) {
				...
				if (data_size) {
					...
					if (stat & I2C_STAT_XRDY)
						printf("XRDY IRQ while no "
							"data to send\n");
					if (stat & I2C_STAT_XDR)
						printf("XDR IRQ while no "
							"data to send\n");
					break;
				}
				outb(data_x, I2C_DATA);
			}
			outw(stat & (I2C_STAT_XRDY | I2C_STAT_XDR), I2C_STAT);
			continue;
		}
	...
	i2c_init(i2c_speed, CFG_I2C_SLAVE);//reset i2c bus when error
	return 1;
 }
