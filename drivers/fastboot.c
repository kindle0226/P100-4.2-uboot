...
*/
	/* Volume up key(GPIO_50) == Enter Recovery */
	gpio_read = __raw_readl(OMAP44XX_CTRL_BASE + CONTROL_PADCONF_GPMC_NCS0);
	__raw_writel((gpio_read | 0x103), OMAP44XX_CTRL_BASE + CONTROL_PADCONF_GPMC_NCS0);
	if (!(__raw_readl(OMAP44XX_GPIO2_BASE + DATA_IN_OFFSET) & (1<<18))){
		printf("Tablet: Volume up key pressed: entering recovery....\n");
		goto start_recovery;
	}
	/* check cam shot key*/
	if(check_fastboot_mode()){
		printf("C1: camera shot key pressed: entering fastboot....\n");
		return 1;
	}
#endif
...
start_recovery:
		...
		} else if (!strcmp(PUBLIC_SAR_RAM_1_FREE, "shipping")) {
			setup_shipping_mode();
