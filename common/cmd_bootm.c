...
int do_booti (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	char locale_str[sizeof(" androidboot.locale=")+LANG_SIZE];
#endif
#ifdef CONFIG_CUSTOM_TIMEZONE_FLASH
	char timezone_str[sizeof(" androidboot.timezone=")+TIME_SIZE];
#endif
	...
	do {
		...
		unsigned char i, j;
		char snbuf[SN_SIZE+1];
		unsigned char keybuf[SN_ENCKEY_SIZE];
		unsigned long chksum_r = 0, chksum_c = 0;

		ptn = fastboot_flash_find_ptn(SN_MMC_PTN);
		if (ptn == 0) {
			serial_len = sprintf(serial_str, " androidboot.serialno=GY20H35795XXXXNOSN");
			break;
		}

		int ret = mmc_init(SN_MMC_SLOT);
		if (ret) {
			serial_len = sprintf(serial_str, " androidboot.serialno=GY20H35795INITFAIL");
			break;
		}

		mmc_read(SN_MMC_SLOT, ptn->start, databuf, 3+SN_SIZE+SN_CHKSUM_SIZE+SN_ENCKEY_SIZE);
		if ((databuf[0]!=SN_SIZE) || (databuf[1]!=SN_CHKSUM_SIZE) || (databuf[2]!=SN_ENCKEY_SIZE)) {
			serial_len = sprintf(serial_str, " androidboot.serialno=GY20H35795LNCKFAIL");
			break;
		}

		for (i=0; i<SN_CHKSUM_SIZE; i++) {
			chksum_r += databuf[3+SN_SIZE+i] << (8*(SN_CHKSUM_SIZE-1-i));
		}

		for (i=0; i<SN_ENCKEY_SIZE; i++) {
			keybuf[i] = databuf[3+SN_SIZE+SN_CHKSUM_SIZE+i]
				^ databuf[3+SN_SIZE+(i%SN_CHKSUM_SIZE)];
		}

		for (i=0; i<SN_SIZE; i++) {
			snbuf[i] = databuf[3+i] ^ keybuf[i%SN_ENCKEY_SIZE];
			if ((i%(SN_CHKSUM_BIT/8)) == ((SN_CHKSUM_BIT/8)-1)) {
				for (j=0; j<(SN_CHKSUM_BIT/8) && j<=i; j++) {
					chksum_c += (unsigned long)(databuf[3+i-j])
						<< (8*((SN_CHKSUM_BIT/8)-1-j));
				}
			}
		}
		snbuf[i]='\0';

		if (chksum_r != chksum_c) {
			serial_len = sprintf(serial_str, " androidboot.serialno=GY20H35795CKSMFAIL");
		} else if (strncmp(keybuf, SN_ENCKEY, SN_ENCKEY_SIZE)) {
			serial_len = sprintf(serial_str, " androidboot.serialno=GY20H35795KYCKFAIL");
		} else {
			serial_len = sprintf(serial_str, " androidboot.serialno=%s", snbuf);
		}
	} while(0);

	memcpy(short_sn, (serial_str+serial_len-4), sizeof(short_sn));
	strcat(serial_str, " androidboot.devname=TS100_");
	strcat(serial_str, short_sn);
	serial_len += 26 + sizeof(short_sn);

	// uboot_version_string was corrupted after SN reading due to unknown reason
	// Restore the string here
	sprintf(uboot_version_string, U_BOOT_VERSION);
#else
	...
	do {
		struct fastboot_ptentry *ptn;
		char databuf[LANG_SIZE];

		ptn = fastboot_flash_find_ptn(LANG_MMC_PTN);
		if (ptn == 0) {
			sprintf(locale_str, " androidboot.locale=%s", DEFAULT_LANG);
			break;
		}

		if (mmc_init(LANG_MMC_SLOT)) {
			sprintf(locale_str, " androidboot.locale=%s", DEFAULT_LANG);
			break;
		}

		mmc_read(LANG_MMC_SLOT, ptn->start+LANG_MMC_OFFSET, databuf, LANG_SIZE);
		if (databuf[2] == '_')
			sprintf(locale_str, " androidboot.locale=%s", databuf);
		else
			sprintf(locale_str, " androidboot.locale=%s", DEFAULT_LANG);
	} while(0);

	if(sizeof(hdr->cmdline) >= (sizeof(locale_str) + strlen(hdr->cmdline) + 1))
		strcat(hdr->cmdline, locale_str);
#endif
#ifdef CONFIG_CUSTOM_TIMEZONE_FLASH
	do {
		struct fastboot_ptentry *ptn;
		char databuf[TIME_SIZE];

		ptn = fastboot_flash_find_ptn(TIME_MMC_PTN);
		if (ptn == 0) {
			sprintf(timezone_str, " androidboot.timezone=%s", DEFAULT_TIME);
			break;
		}

		if (mmc_init(TIME_MMC_SLOT)) {
			sprintf(timezone_str, " androidboot.timezone=%s", DEFAULT_TIME);
			break;
		}

		mmc_read(TIME_MMC_SLOT, ptn->start+TIME_MMC_OFFSET, databuf, TIME_SIZE);
		if (databuf[6] == ':')
			sprintf(timezone_str, " androidboot.timezone=%s", databuf);
		else
			sprintf(timezone_str, " androidboot.timezone=%s", DEFAULT_TIME);
	} while(0);

	if(sizeof(hdr->cmdline) >= (sizeof(timezone_str) + strlen(hdr->cmdline) + 1))
		strcat(hdr->cmdline, timezone_str);
#endif
